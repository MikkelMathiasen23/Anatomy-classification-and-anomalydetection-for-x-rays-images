...
def train_model(config):
    ...
    train_loader=DataLoader(dataset=train_mura_dataset,batch_size=config['batch'],num_workers=0, shuffle = True )
    val_loader=DataLoader(dataset=val_mura_dataset,batch_size=config['batch'],num_workers=0, shuffle = True )
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            #os.chdir('/content/drive/My Drive/Kvantitativ biologi og sygdomsmodellering/Bachelor project')
            # Iterate over data.
            for inputs, labels_anomaly, labels_anatomy in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    preds_anatomy = torch.max(outputs['anatomy'],1)[1]#outputs['anatomy']
                ...
                #labels_anatomy = torch.max(labels_anatomy, 1)[1]
                ...
                #running_corrects_anatomy += f1_score(labels_anatomy, preds_anatomy, average = 'weighted')
            ...
            anomaly_standard_error = np.std(SE_anomaly)/np.sqrt((len(dataloaders[phase].dataset)))  
            anatomy_standard_error = np.std(SE_anatomy)/np.sqrt((len(dataloaders[phase].dataset)))
...
analysis = tune.run(train_model, config={"lr": tune.grid_search([0.001, 0.01, 0.1]), "batch": tune.grid_search([64,128,256,])}, resources_per_trial={'gpu':2})
